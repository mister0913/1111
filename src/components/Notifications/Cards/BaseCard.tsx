...
const Container = styled(Row)<{ bg?: string; border?: string; report?: boolean; cursor?: string }>`
  background: ${({ theme, bg }) => (bg ? bg : theme.color4)};
...
const AccountName = styled(RowEnd)<{ alert?: string }>`
  ...
  font-size: 12px;
  width: unset;
  color: ${({ theme, alert }) => (alert ? alert : theme.almostWhite)};
`
      ...
      >
        <LogoWrapper rotate={rotate}>
          {loading ? (
            ...
            <>
              {icon && <Image src={icon} width={getImageSize()} height={getImageSize()} alt={`icon`} />}
              {(token1 || token2) && <Logos img1={token1 ?? DefaultToken} img2={token2 ?? DefaultToken} />}
            </>
          )}
        </LogoWrapper>
        ...
        <Wrapper>
          <TextRow style={{ marginBottom: '8px' }}>
            ...
            <RowEnd>
              <AccountName alert={!!border ? theme.text1 : undefined}>{`[${accountName}]`}</AccountName>
              <Timestamp alert={!!border ? theme.text0 : undefined}>{time}</Timestamp>
            </RowEnd>
          </TextRow>
          <TextRow>
            {text}
