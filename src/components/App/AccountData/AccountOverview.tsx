...
import { useAccountPartyAStat, useActiveAccount, useExpertMode, useSetExpertModeCallback } from 'state/user/hooks'
...
export default function AccountOverview({ mobileVersion = false }: { mobileVersion?: boolean }) {
  ...
  if (validatedContext !== ContextError.VALID) {
    ...
    return (
      <>
        <Wrapper>
          <TopRow>
            <DeveloperModeTitle title={mobileVersion ? 'Account Overview' : accountName} />
  ...
  return (
    <Wrapper>
      <ColumnCenter style={{ marginTop: '78px' }}>
        <Image src={'/static/images/etc/SimpleLogo.svg'} alt="Asset" width={110} height={120} />
...
export const DeveloperModeTitle = ({ title }: { title: string | undefined }) => {
  ...
  useEffect(() => {
    ...
    if (tries >= 5) {
      ...
      toast.error(`Developer mode is ${isExpertMode ? 'deactivating' : 'activating'} #${tries}`)
    }
