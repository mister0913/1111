...
import { DEFAULT_PRECISION, MAX_PENDINGS_POSITIONS_NUMBER, MARKET_PRICE_COEFFICIENT } from 'constants/misc'
import { removeTrailingZeros, RoundMode, toBN } from 'utils/numbers'
import { InputField, OrderType, ErrorState, PositionType } from 'types/trade'
...
export default function useTradePage(): {
  ...
  }, [orderType, limitPrice, marketPrice, slippage, positionType, autoSlippage])
  ...
  const outOfRangePrice = useMemo(() => {
    ...
    if (orderType === OrderType.LIMIT && market && market.name === name) {
      if (positionType === PositionType.LONG) {
        return (
          toBN(limitPrice).isLessThan(minPrice) ||
          toBN(limitPrice).isGreaterThan(toBN(marketPrice).times(MARKET_PRICE_COEFFICIENT))
        )
      } else {
        return (
          toBN(limitPrice).isGreaterThan(maxPrice) ||
          toBN(limitPrice).isLessThan(toBN(marketPrice).div(MARKET_PRICE_COEFFICIENT))
        )
      }
    }
    return false
