...
import { useAddQuotesToListenerCallback, useListenersQuotes, usePendingsQuotes, usePositionsQuotes } from './hooks'
...
export function QuotesUpdater(): null {
  ...
  useEffect(() => {
    if (account && chainId)
      return autoRefresh(() => thunkDispatch(getHistory({ account, chainId, first: 8, skip: 0 })), 3000)
